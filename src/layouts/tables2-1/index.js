...
function ServiceTypes() {
  ...
  async function fetchData() {
    try {
      ...
      const { data: tableData, error: fetchError } = await supabase
        .from("service_type")
        .select("*, service:service_id(id, name)")
        .eq("is_active", true)
  ...
  const handleEditClick = (row) => {
    ...
    setEditFormTouched({ name: false, description: false, service_id: false });
    if (row.illustration) {
  ...
  const handleAddClick = () => {
    ...
    setAddFormTouched({ name: false, description: false, service_id: false });
  ...
  const handleAddSave = async () => {
    try {
      ...
      const insertData = {
        ...
        service_id: addFormData.service_id || null,
        is_active: true,
        created_at: new Date().toISOString(),
        created_by: "admin",
      };

      const { error: insertError } = await supabase.from("service_type").insert(insertData);
  ...
  return (
    <DashboardLayout>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle id="edit-dialog-title">Edit Service Type</DialogTitle>
        <DialogContent>
          <MDBox pt={2}>
            <FormControl
              fullWidth
            ...
            >
              <InputLabel id="edit-service-label">Service *</InputLabel>
              <Select
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle id="add-dialog-title">Add Service Type</DialogTitle>
        <DialogContent>
          <MDBox pt={2}>
            <FormControl
              fullWidth
            ...
            >
              <InputLabel id="add-service-label">Service *</InputLabel>
              <Select
