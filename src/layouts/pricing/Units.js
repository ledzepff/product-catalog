...
function Units() {
  ...
  // localStorage keys
  const COLUMN_ORDER_KEY = "units-column-order";
  const HIDDEN_COLUMNS_KEY = "units-hidden-columns";
  ...
  const handleEditClick = (row) => {
    ...
    setEditFormData({
      name: row.name || "",
      symbol: row.symbol || "",
      multiplier_to_base: row.multiplier_to_base || "",
    });
    setEditFormTouched({});
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const updateData = {
        ...
        multiplier_to_base: editFormData.multiplier_to_base ? parseFloat(editFormData.multiplier_to_base) : null,
        updated_at: new Date().toISOString(),
      };
  ...
  const handleAddClick = () => {
    setAddFormData({ name: "", symbol: "", multiplier_to_base: "" });
    setAddFormTouched({});
  ...
  const handleAddSave = async () => {
    try {
      ...
      const insertData = {
        ...
        symbol: addFormData.symbol || null,
        multiplier_to_base: addFormData.multiplier_to_base ? parseFloat(addFormData.multiplier_to_base) : null,
        created_at: new Date().toISOString(),
      };

      const { error: insertError } = await supabase.from("units").insert(insertData);
  ...
  const filteredData = data.filter((row) => {
    ...
    return (
      ...
      (row.symbol && row.symbol.toLowerCase().includes(search))
    );
  });
  ...
  const allColumns = useMemo(() => [
    ...
    },
    {
      ...
      width: "20%",
      sortType: (rowA, rowB) => {
        const a = rowA.original.symbol_raw || "";
        const b = rowB.original.symbol_raw || "";
        return a.localeCompare(b);
      },
    },
    {
      Header: "Multiplier to Base",
      accessor: "multiplier_to_base",
      id: "multiplier_to_base",
      width: "25%",
      sortType: (rowA, rowB) => {
        const a = rowA.original.multiplier_to_base_raw || 0;
        const b = rowB.original.multiplier_to_base_raw || 0;
        return a - b;
      },
    },
    {
  ...
  const rows = filteredData.map((row) => ({
    ...
    multiplier_to_base: (
      <MDTypography variant="caption" color="text">
        {row.multiplier_to_base != null ? row.multiplier_to_base : "-"}
      </MDTypography>
    ),
    multiplier_to_base_raw: row.multiplier_to_base || 0,
    actions: (
        ...
        <DialogContent>
          <MDBox pt={2}>
            ...
            <TextField
              ...
              margin="normal"
              placeholder="e.g., GB, ms, %"
            />
            <TextField
              fullWidth
              label="Multiplier to Base"
              type="number"
              value={editFormData.multiplier_to_base || ""}
              onChange={(e) => handleEditFormChange("multiplier_to_base", e.target.value)}
              margin="normal"
              placeholder="e.g., 1024 for KB to bytes"
            />
          </MDBox>
        </DialogContent>
        <DialogActions>
        ...
        <DialogContent>
          <MDBox pt={2}>
            ...
            <TextField
              ...
              label="Multiplier to Base"
              type="number"
              value={addFormData.multiplier_to_base}
              onChange={(e) => handleAddFormChange("multiplier_to_base", e.target.value)}
              margin="normal"
              placeholder="e.g., 1024 for KB to bytes"
            />
          </MDBox>
