...
function MeterValueTypes() {
  ...
  // localStorage keys
  const COLUMN_ORDER_KEY = "meter-value-types-column-order";
  const HIDDEN_COLUMNS_KEY = "meter-value-types-hidden-columns";

  async function fetchData() {
    try {
      setLoading(true);
      const { data: tableData, error: fetchError } = await supabase
        .from("meter_value_types")
        .select("*")
        .is("deleted_at", null)
  ...
  const handleAddSave = async () => {
    try {
      setAdding(true);
      const { error: insertError } = await supabase.from("meter_value_types").insert({
  ...
  return (
    <DashboardLayout>
                ...
                <MDBox px={3} pb={2}>
                  <MDBox display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                    <MDBox display="flex" alignItems="center" gap={2}>
                      <TextField
                        placeholder="Search value types..."
                        size="small"
                        value={searchTerm}
                  ...
                  {loading ? (
                    ...
                    <MDBox px={3} pb={3}>
                      <MDTypography variant="body2" color="text">
                        No meter value types found. Click &quot;Add Value Type&quot; to create one.
                      </MDTypography>
                    </MDBox>
                  ) : (
                    <DataTable
                      ...
                      tableId="meter-value-types-table"
                    />
                  )}
                </MDBox>
              </MDBox>
      ...
      <Dialog open={editDialogOpen} onClose={handleEditClose} maxWidth="sm" fullWidth>
        <DialogTitle>Edit Meter Value Type</DialogTitle>
        <DialogContent>
          <MDBox pt={2}>
            <TextField
              fullWidth
              ...
              placeholder="e.g., Counter, Gauge, Delta"
            />
          </MDBox>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={addDialogOpen} onClose={handleAddClose} maxWidth="sm" fullWidth>
        <DialogTitle>Add Meter Value Type</DialogTitle>
        <DialogContent>
          <MDBox pt={2}>
            <TextField
              fullWidth
...
export default MeterValueTypes;
